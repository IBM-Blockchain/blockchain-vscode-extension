...
export async function addGateway(): Promise<{} | void> {
    ...
    try {
        ...
        if (gatewayMethod === UserInputUtil.ADD_GATEWAY_FROM_ENVIRONMENT) {
            chosenEnvironment = await UserInputUtil.showFabricEnvironmentQuickPickBox('Choose an environment to create a gateway from', false, false) as IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry>;
