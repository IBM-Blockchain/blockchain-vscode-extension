...
describe('PackageSmartContract', () => {
    ...
    describe('packageContract', () => {
        it('should package a smart contract', async () => {
            ...
            const files: string[] = await PackageSmartContract.packageContract('myContract', '0.0.1', contractPath, pkgFile, 'node', null);
        ...
        it('should package a smart contract with meta data', async () => {
            ...
            const files: string[] = await PackageSmartContract.packageContract('myContract', '0.0.1', contractPath, pkgFile, 'node', metadataPath);
            files.should.deep.equal(['META-INF/statedb/couchdb/indexes/indexOwner.json', 'src/chaincode.js', 'src/chaincode.ts', 'src/contract-metadata/statedb/couchdb/indexes/indexOwner.json', 'src/package.json']);
        });
    });

    describe('getPackageInfo', () => {
        it('should get the info from a package', async () => {
            const pkgFile: string =  path.join(rootPath, 'test', 'data', 'packages', 'vscode-pkg-1@0.0.1.cds');
            const pkgBuffer: Buffer = await fs.readFile(pkgFile);
            const info: {name: string, version: string } = await PackageSmartContract.getPackageInfo(pkgBuffer);
            info.should.deep.equal({name: 'vscode-pkg-1', version: '0.0.1'});
        });
    });
});
