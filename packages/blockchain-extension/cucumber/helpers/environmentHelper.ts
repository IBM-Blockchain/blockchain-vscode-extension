...
export class EnvironmentHelper {
    ...
    public async createEnvironment(name: string, opsType?: string, createLocal?: boolean, capability: string = UserInputUtil.V2_0): Promise<IBlockchainQuickPickItem<FabricNode>[]> {
        ...
        if (!treeItem) {
            ...
            if (process.env.OPSTOOLS_FABRIC) {
                ...
                this.userInputUtilHelper.selectCapabilitiesStub.withArgs('Select the channel capability version to use for the network').resolves(capability);
            } else if (createLocal) {
                this.userInputUtilHelper.showQuickPickItemStub.withArgs('Select a method to add an environment').resolves({data: UserInputUtil.ADD_ENVIRONMENT_FROM_TEMPLATE});
