...
let carid: string;
let ownerSequenceNum: number = 0;
const DELETED_CAR_OWNER_IDENTIFIER: string = 'CAR KEY DELETED';
...
async function main(): Promise<void> {
  try {
    ...
    const existsBuffer: Buffer = await contract.evaluateTransaction('carExists', carid);
    ...
    const carDetailsBuffer: Buffer = await contract.evaluateTransaction('queryCar', carid);
    const car: Car = JSON.parse(carDetailsBuffer.toString()) as Car;
    const previousOwnersBuffer: Buffer = await contract.evaluateTransaction('getPreviousOwners', carid);
    ...
    if (history.previousOwnerCount > 0) {
      for (let i: number = history.previousOwners.length - 1; i >= 0; i--) {
        const txTime: string = new Date(history.previousOwnershipChangeDates[i]).toUTCString();
    ...
    const txTime: string = new Date(history.currentOwnershipChangeDate).toUTCString();
...
function handleEvent(error: Error, event: any): void {
  if (error) {
  ...
  if (ownerEvent.carNumber === carid) {
    const txTime: string = new Date(ownerEvent.transactionDate).toUTCString();
