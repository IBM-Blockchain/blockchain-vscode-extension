...
export async function startFabricRuntime(registryEntry?: RuntimeTreeItem | FabricEnvironmentRegistryEntry): Promise<void> {
    ...
    if (!registryEntry) {
        const chosenEnvironment: IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry> = await UserInputUtil.showFabricEnvironmentQuickPickBox('Select an environment to start', false, true, [EnvironmentFlags.MANAGED_MICROFAB], [], false) as IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry>;
