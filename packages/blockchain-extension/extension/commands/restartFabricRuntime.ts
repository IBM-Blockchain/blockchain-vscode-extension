...
export async function restartFabricRuntime(runtimeTreeItem?: RuntimeTreeItem): Promise<void> {
    ...
    if (!runtimeTreeItem) {
        ...
        if ((registryEntry && !registryEntry.managedRuntime) || !registryEntry) {
            const chosenEnvironment: IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry> = await UserInputUtil.showFabricEnvironmentQuickPickBox('Select an environment to restart', false, true, [EnvironmentFlags.MANAGED_MICROFAB], [], true) as IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry>;
