...
export async function deleteNode(nodeTreeItem: NodeTreeItem | FabricNode, hideNode?: boolean): Promise<void> {
    ...
    try {
        ...
        if (!nodeTreeItem) {
            ...
            // First check there is at least one that isn't ansible or opstools
            const environments: Array<FabricEnvironmentRegistryEntry> = await FabricEnvironmentRegistry.instance().getAll([EnvironmentFlags.ENVIRONMENT]);
