...
export async function stopFabricRuntime(runtimeTreeItem?: RuntimeTreeItem): Promise<void> {
    ...
    if (!runtimeTreeItem) {
        ...
        if ((registryEntry && !registryEntry.managedRuntime) || !registryEntry) {
            const chosenEnvironment: IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry> = await UserInputUtil.showFabricEnvironmentQuickPickBox('Select an environment to stop', false, true, [EnvironmentFlags.MANAGED_MICROFAB], [], true) as IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry>;
