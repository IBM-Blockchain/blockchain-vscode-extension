...
export async function createSmartContractProject(): Promise<void> {
    ...
    contractType = await UserInputUtil.showQuickPick('Choose a contract type to generate:', [{label: 'Default Contract', description: 'Demonstrates CRUD operations to a ledger shared by all network members'}, {label: 'Private Data Contract', description: 'Demonstrates CRUD + verify operations to a private data collection'}]) as IBlockchainQuickPickItem<string>;

    if (!contractType) {
        return;
        // User has cancelled the QuickPick
    }
    if (contractType.label === 'Default Contract') {
        contractType.data = 'standard';
        privateOrDefault = ' ';
    } else {
    ...
    if (smartContractLanguageItem.type === LanguageType.CONTRACT) {
        if (contractType.data === 'standard') {
            assetType = await UserInputUtil.showInputBox('Name the type of asset managed by this smart contract', 'MyAsset');
        } else {
            assetType = await UserInputUtil.showInputBox('Name the type of asset managed by this smart contract', 'MyPrivateAsset');
        }
