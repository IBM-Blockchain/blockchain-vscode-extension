...
import {  FabricEnvironmentRegistry, FabricEnvironmentRegistryEntry, EnvironmentType, FabricNode, FabricNodeType, FabricWalletRegistry, FabricWalletRegistryEntry, IFabricCertificateAuthority, IFabricWallet, IFabricWalletGenerator, LogType, FabricEnvironment, FabricWalletGeneratorFactory, EnvironmentFlags } from 'ibm-blockchain-platform-common';
...
export async function associateIdentityWithNode(replace: boolean = false, environmentRegistryEntry: FabricEnvironmentRegistryEntry, node: FabricNode): Promise<any> {
    ...
    try {
        ...
        if (!environmentRegistryEntry || !node) {
            ...
            const environments: Array<FabricEnvironmentRegistryEntry> = await FabricEnvironmentRegistry.instance().getAll([], [EnvironmentFlags.MANAGED_MICROFAB]);
