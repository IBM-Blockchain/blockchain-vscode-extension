...
export class UserInputUtil {
    ...
    public static async showChaincodeAndVersionQuickPick(prompt: string, channel: string, peers: Array<string>, contractName?: string, contractVersion?: string): Promise<IBlockchainQuickPickItem<{ packageEntry: PackageRegistryEntry, workspace: vscode.WorkspaceFolder }> | undefined> {
        ...
        const instantiatedSmartContracts: Array<FabricSmartContractDefinition> = await connection.getCommittedSmartContractDefinitions(peers, channel);
    ...
    public static async showClientInstantiatedSmartContractsQuickPick(prompt: string, channelName?: string, showAssociated?: boolean): Promise<IBlockchainQuickPickItem<{ name: string, channel: string, version: string }> | undefined> {
        ...
        if (showAssociated === true) {
            ...
            if (transactionDataDirectories === undefined || transactionDataDirectories.length === 0) {
                ...
                const tempChaincodes: Array<{ name: string, version: string, sequence: number, channel: string }> = instantiatedChaincodes.filter((chaincode: FabricSmartContractDefinition) => {
    ...
    public static async showRuntimeInstantiatedSmartContractsQuickPick(prompt: string, channelName?: string): Promise<IBlockchainQuickPickItem<{ name: string, channel: string, version: string }> | undefined> {
        ...
        for (const [thisChannelName, peerNames] of channelMap) {
            ...
            const chaincodes: Array<FabricSmartContractDefinition> = await connection.getCommittedSmartContractDefinitions(peerNames, thisChannelName); // returns array of objects
            for (const chaincode of chaincodes) {
