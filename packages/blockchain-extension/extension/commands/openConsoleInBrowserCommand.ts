...
import { FabricEnvironmentRegistryEntry, LogType, IFabricEnvironmentConnection, EnvironmentType } from 'ibm-blockchain-platform-common';
    ...
    try {
        if (!environment) {
            ...
            if (!cloudEnvironment || !cloudEnvironment.environmentType || (cloudEnvironment.environmentType !== EnvironmentType.OPS_TOOLS_ENVIRONMENT && cloudEnvironment.environmentType !== EnvironmentType.SAAS_OPS_TOOLS_ENVIRONMENT)) {
                ...
                const chosenEnvironment: IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry> = await UserInputUtil.showFabricEnvironmentQuickPickBox('Choose an IBM Blockchain Platform environment to open in browser', false, false, false, IncludeEnvironmentOptions.OPSTOOLSENV) as IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry>;
