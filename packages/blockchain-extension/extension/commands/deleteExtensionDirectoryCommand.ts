...
import { FabricEnvironmentRegistryEntry, LogType, FabricEnvironmentRegistry } from 'ibm-blockchain-platform-common';
...
export async function deleteExtensionDirectory(): Promise<void> {
    ...
    try {
        if (!await UserInputUtil.showConfirmationWarningMessage(`This will delete the extension directory. Do you want to continue?`)) {
            return;
        }
