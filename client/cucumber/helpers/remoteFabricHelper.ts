...
export class RemoteFabricHelper {

    private client: Client;

    private testPath: string = path.join(__dirname, '..', '..', '..', 'cucumber');

    public async connect(): Promise<void> {
        const filePath: string = path.join(this.testPath, 'hlfv1', 'connection.json');
        ...
        const networkAdminPrivateKeyPath: string = path.resolve(networkAdminPath, 'msp/keystore/key1.pem');
    ...
    public async installChaincode(name: string, version: string): Promise<void> {
        const peer: Client.Peer = this.client.getPeersForOrg('Org1MSP')[0];
        const filePath: string = path.join(this.testPath, 'tmp', 'packages', `${name}@${version}.cds`);
    ...
    public async instantiateChaincode(name: string, version: string): Promise<void> {
