...
export async function installSmartContract(peerTreeItem?: PeerTreeItem, peerNames?: Set<string>): Promise<PackageRegistryEntry | boolean> {
    ...
    try {
        ...
        for (const peer of peerNames) {
            const install: Promise<void> = fabricClientConnection.installChaincode(chosenPackage.data, peer).catch((error) => {
                return error.message; // We return the error message so we can display it to the user
            });
        ...
        await Promise.all(promises).then(async (result) => {
            ...
            for (const peer of peerSet) {
                if (!result[counter]) {
                    await vscode.window.showInformationMessage(`Successfully installed on peer ${peer}`);
                    outputAdapter.log(`Successfully installed on peer ${peer}`);
                    counter++;
                } else {
                    successfulInstall = false; // Install on peer failed
                    await vscode.window.showErrorMessage(`Failed to install on peer ${peer} with reason: ${result[counter]}`);
        ...
        if (successfulInstall) {
            ...
            await vscode.window.showInformationMessage('Successfully installed smart contract');
