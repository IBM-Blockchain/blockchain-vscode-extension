...
export async function startFabricRuntime(): Promise<void> {
    ...
    await vscode.window.withProgress({
        ...
        progress.report({ message: `Starting Fabric runtime ${fabricRuntimeName}` });
        try {
            ...
            outputAdapter.log(LogType.ERROR, `Failed to start Local Fabric: ${error.message}`, `Failed to start Local Fabric: ${error.toString()}`);
        }
