...
export async function associateIdentityWithNode(replace: boolean = false, environmentRegistryEntry: FabricEnvironmentRegistryEntry, node: FabricNode): Promise<any> {
    ...
    try {
        ...
        if (!environmentRegistryEntry || !node) {
            ...
            const chosenEnvironment: IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry> = await UserInputUtil.showFabricEnvironmentQuickPickBox('Choose an environment to use', false) as IBlockchainQuickPickItem<FabricEnvironmentRegistryEntry>;
